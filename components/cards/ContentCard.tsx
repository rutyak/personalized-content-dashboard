    ...
    >
      {!tending && (
        <button
          ...
          className={`absolute z-[100] p-2 rounded-full backdrop-blur-md border transition-all active:scale-90 right-4 ${
      ...
      <div className="flex flex-col h-full">
        {!isSocial && (
          <div
            className={`relative overflow-hidden shrink-0 w-full transition-all duration-500 ${isMovie ? "absolute inset-0 h-full sm:h-96 md:h-full" : "h-48 lg:h-56"}`}
          >
            <img
              src={displayImage}
              alt={title}
            ...
            />
            <div
              className={`absolute inset-0 ${isMovie ? "bg-gradient-to-t from-black/95 via-black/60 to-transparent" : "bg-black/5 md:bg-transparent"}`}
            />
          </div>
        )}
